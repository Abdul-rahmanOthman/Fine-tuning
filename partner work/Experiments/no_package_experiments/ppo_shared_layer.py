"""
This code modifies the shared layer on every networks own loss, WITHOUT adding the shared layer to the loss of the other network.
"""
DEVICE = 'cuda'
import random
import torch
...
class Manager():
  def __init__(self, dataset= None, action_list =None ,symptoms_list=None, disease_list=None, input_dim = None, config=None ):
    ...
    self.trainer = Trainer( self.agent, policy_lr = policy_lr, value_lr = value_lr, target_kl_div = 0.02, \
                           max_policy_train_iters = 40, value_train_iters = 40, symptoms_list= symptoms_list,\
                            action_set= action_list, disease_list=disease_list, input_dim= self.input_dim,  entropy_coeff = ent_coeff, ppo_clip_val = ppo_clip)
  ...
  def train_loop(self):
    ...
    for episode_idx in range(self.train_episodes):
        ...
        if (episode_idx + 1) % self.batch_size == 0 or (episode_idx + 1) == self.train_episodes:
            # Shuffle data
