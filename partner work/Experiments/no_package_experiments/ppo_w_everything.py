...
2-  Feature Engineering of the disease
3-  RMSProp optimizer is used for the actor and critic networks
4-  Editing the choose_action() method a little bit to threshold actions
5-  input_dims = 3* num_of_symptoms + num_of_diseases +  2
6-  used MD dataset # change later
...
# make a dictionary for each each disease its correspondent symptoms that occured in the dataset goals without repetetion
for disease in disease_list:
  disease_symptoms[disease] = []

for goal in dataset:
  disease = goal['disease_tag']
  symptoms = list(goal['explicit_inform_slots'].keys()) + list(goal['implicit_inform_slots'].keys())
  for symptom in symptoms:
    if symptom not in disease_symptoms[disease]:
...
class StateTracker():
  def __init__(self, symptoms_list, disease_list, disease_symptoms):
  ...
  def update(self, agent_action=None, user_action=None):
      ...
      if user_action:
        #print("The update happened and the inform slots are : ", user_action['inform_slots'].items())
  ...
  def get_state(self):
    # Represent current slots as a sparse vector

    disease_vector = self.calculate_disease_vector(self.current_slots['inform_slots'])
    ...
    # Represent last agent action using embeddings
  ...
  def calculate_disease_vector(self, current_slots):
    '''
    current_slots['inform slots']: dict = {symptom: value, ...}
    '''

    # Initialize the disease vector with zeros
    disease_vector = torch.zeros( len(self.disease_list))
    
    # Get the total number of symptoms in current_slots
    num_slots = len(self.symptoms_list)
    
    # Iterate over each disease
    for i, (disease, symptoms) in enumerate(self.disease_symptoms.items()):
        # Initialize the score for the current disease
        disease_score = 0.0
        
        # Iterate over each symptom in the disease's symptom list
        for symptom in symptoms:
            if symptom in list(current_slots.keys()):
                value = current_slots[symptom]
                if value == 3.0:
                    disease_score += 3 / num_slots
                elif value == 2.0:
                    disease_score += 2 / num_slots * 0.5
        
        # Assign the calculated score to the corresponding position in the disease vector
        disease_vector[i] = disease_score
        #convert the disease_vector dictionary to a tensor
        disease_vector = torch.tensor(disease_vector, dtype=torch.float32, device=DEVICE)
...
# choosing action with threshold and confidence
  def choose_action(self, observation, turn):
    ...
    prob = dist.probs  # assuming dist is a Categorical distribution, otherwise adjust accordingly
    # Get the last 5 output neurons
    last_5_outputs = prob[-5:]
    prob_except_last_5 = prob[:-5]

    max_last_5 = torch.max(last_5_outputs).item() # Get the maximum value for disease
    max_index_last_5 = torch.argmax(last_5_outputs).item() # get the index of the maximum value of disease
    max_except_last_5 = torch.max(prob_except_last_5).item()


    if turn >= self.max_turn:
        action = (self.n_actions - 5) + max_index_last_5
        action_tensor = torch.tensor(action).to(prob.device)  # Convert action to tensor
        
    else:
        threshold = 0.8 * float(5 / self.n_actions)

        if max_last_5 >= threshold:
          action = (self.n_actions - 5) + max_index_last_5
          action_tensor = torch.tensor(action).to(prob.device)  # Convert action to tensor
          ...
          # Exclude the last 5 actions
          '''
          prob_except_last_5 = prob[:-5]
          # Find the index of the most probable action excluding the last 5 actions
          max_except_last_5 = torch.max(prob_except_last_5).item()
          max_index_except_last_5 = torch.argmax(prob_except_last_5).item()
          action = max_index_except_last_5
          action_tensor = torch.tensor(action).to(prob.device)  # Convert action to tensor
         #print("The action taken is the most probable excluding last 5 with confidence ", max_except_last_5, "with action ", action_list[action])
          '''
...
class Manager:
  def __init__(self, dataset= None, action_list =None ,symptoms_list=None, disease_list=None, input_dim = None, config=None ):
    ...
    self.state_tracker = StateTracker(symptoms_list, disease_list, disease_symptoms)
...
def run_training(dataset, action_list, symptoms_list, disease_list, config):
    ...
    input_dims =  3* num_of_symptoms + num_of_diseases +  2
    ...
    global Train
    Train = False
    print("Test Accuracy: ", manager.test_accuracy(test_size))
    return accuracy
