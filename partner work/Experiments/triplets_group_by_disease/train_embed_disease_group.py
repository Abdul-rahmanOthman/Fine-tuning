...
ix_grouped = ix_grouped[:-1]


dialogue_disease_mapping = {}

for group in ix_grouped:
    disease_idx = group[-1]
    disease = actions[disease_idx]
    
    if disease not in dialogue_disease_mapping:
        dialogue_disease_mapping[disease] = []
        
    dialogue_disease_mapping[disease].extend(group)

dialogue_disease_list = copy.deepcopy(list(dialogue_disease_mapping.values()))

ix_grouped = copy.deepcopy(dialogue_disease_list)
...
for ix, action in enumerate(actions_test):
  if actions_test[ix] == 'disease':
    ix_grouped_test[-1].append(ix)
    ix_grouped_test.append([])
  else:
    ix_grouped_test[-1].append(ix)

ix_grouped_test = ix_grouped_test[:-1]

dialogue_disease_mapping_test = {}

for group in ix_grouped_test:
    disease_idx = group[-1]
    disease = actions[disease_idx]
    
    if disease not in dialogue_disease_mapping_test:
        dialogue_disease_mapping_test[disease] = []
        
    dialogue_disease_mapping_test[disease].extend(group)

dialogue_disease_list_test = list(dialogue_disease_mapping_test.values())

ix_grouped_test = copy.deepcopy(dialogue_disease_list_test)
